...
class DossierAssignmentController extends Controller
{
    ...
    {
        $unassigned_dossiers = dossier::join('dossier_status_lookups','dossier_status_lookups.id',
        ...
        ->select('dossiers.*','applications.application_id','applications.application_type',
...
->select('dossiers.*','dossier_assignments.id as dossier_assignment_id','applications.application_id','applications.application_type','medicinal_products.product_trade_name')
->get();

$breadcrumb_title = 'Assigned Dossiers';
        $breadcrumb_title = 'Unassigned Dossiers';
...
//        
    ...
    public function assign_dossier(Request $request)
    {
        ...
        $assigned_datetime = date('Y-m-d H:i:s', strtotime('-3'));
        ...
        elseif($application->application_type==2){
                ...
                $dossier_ref_num = $this->generate_dossier_ref($dossier_id, $track_type);
    ...
    public function reassign_dossier(Request $request)
    {
        ...
        $alert_before_days = 10;
                ...
                $new_notification = [];
                $new_notification['type'] = 'Notification';
                $new_notification['data'] = 'Dossier Reassigment. Dossier Ref. Num: '. $dossier_ref_num;
        ...
        return redirect('/dossier_assignment/assigned')->with('success', 'Dossier Reassigned Successfully.');
    }
    ...
    private function generate_dossier_ref($dossier_id,$track_type)
    {
    ...
    public function retrieve_assessor_assignments(Request $request)
    {
        ...
        try {
            ...
            $return_data = "";
            $assessor_check = dossier_assignment::where('assessor_id', $id)->get();
            if (count($assessor_check) > 0) {
                $assessor_assignment_details = dossier_assignment::where('assessor_id', $id)
                    ->join('users', 'users.id', 'dossier_assignments.assessor_id')
                    ->join('dossiers', 'dossiers.id', 'dossier_assignments.dossier_id')
                    ->whereIn('dossiers.assignment_status',[2,3])
                ...
                foreach ($assessor_assignment_details as $assignment) {
                    ...
                    $return_data .= "<td>" . $application->applicant_id . "</td>";
                    ...
                    $return_data .= "<td>".$main_task->task_status."</td>";
                    $return_data .= "<td><a href='/dossier_evaluation/".$assignment->id."/edit ' class='btn btn-info btn-sm'  ><i class='fas fa-list'></i></a></td></tr> ";
                }
            ...
            return response()->json(['assignment' => $e, 'data'=>$assessor_assignment_details, 'item' => 'error' . $e]);
        }
