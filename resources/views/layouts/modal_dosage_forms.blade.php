            ...
            <div class="modal-body">
                <form id="Dosage_Forms" name="Dosage_Forms" class="form-horizontal">
                   <input type="hidden" name="dosage_id" id="dosage_id">
                    ...
                    <label  class="">Dosage Name</label>
                    <div class="col-sm-12">
                    <input type="text" class="form-control" id="dosage_name" name="name" placeholder="Dosage Name" value="" maxlength="50" required="">
                    </div>
                    </div>
...
function Toastr()
  
  {
    ...
    $('body').on('click', '.editdosage', function () {
      ...
      $('#Dosage_id').removeClass("blue");
      $.get("{{ route('dosageforms.index') }}" +'/' + dosage_id +'/edit', function (data) {
          $('#modelHeading').html("Edit Dosage");
...
toastr.error(data.statusText+"Fill all Provided fields correctly")
    ...
    $('body').on('click', '.deletedosage', function () {
        ...
        $.ajax({
            type: "DELETE",
            url: "{{ route('dosageforms.store') }}"+'/'+dosage_id,
            success: function (data) {
                Toastr();
