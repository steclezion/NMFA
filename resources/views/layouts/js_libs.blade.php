...
function filevalidiator(error,file_id,response_id,valid_file_type_list)
{
if ($('input:submit').attr('disabled',false)){
	$('input:submit').attr('disabled',true);
    }
...
if ($.inArray(ext, valid_file_type_list) == -1){
    document.getElementById(error).innerHTML="Invalid File Format! Allowed File format: "+valid_file_type_list;
	$('#'+error).slideDown("slow");
    $('#error2').slideUp("slow");
    $('#error3').slideUp("slow");
    $('#'+response_id).attr('disabled',true);
	a=0;
	}else{
        $('#'+response_id).show('10');
    //var picsize = ($('#'+file_id).files[0].size);
	...
	if (a==1){
        $('input:submit').attr('disabled',false);
        $('#'+response_id).attr('disabled',false);
		}
}
}
