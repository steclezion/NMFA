...
<!---------------------------------- Section 4 -->
          ...
          <input hidden type="text" value= "{{ $application_id->application_id }}"  id="app_id"/>
            ...
            <tr><td {{ $i=1 }}>Route of Administration </td> <td>@foreach ($product_details as $route_admin)  {{  $route_admin->route_name}}<br>@endforeach</td></tr>
            <tr><td {{ $i=1 }}>Storage Condition   </td> <td> @foreach ($product_details as $storage_condition){{ $storage_condition->storage_condition}}  <br> @endforeach </td></tr>
          
            <tr><td {{ $i=1 }}>Shelf life Amount </td> <td>@foreach ($product_details as $shelf_life_amount){{  $shelf_life_amount->shelf_life_amount   }}<br>@endforeach</td></tr>
            <tr><td {{ $i=1 }}>Pharmacotherapeutic Classification (Anatomic-Therapeutic Classification system):   </td> <td> @foreach ($product_details as $Pharmacotherapeutic){{ $Pharmacotherapeutic->pharmaco_therapeutic_classification    }}<br> @endforeach </td></tr>
          
            <tr><td {{ $i=1 }}>Proposed Shelf Life Amount </td> <td>@foreach ($product_details as $proposed_shelf_life_amount){{  $proposed_shelf_life_amount->proposed_shelf_life_amount   }}<br>@endforeach</td></tr>
            <tr><td {{ $i=1 }}>Proposed Shelf Life After Reconstitution Amount   </td> <td> @foreach ($product_details as $proposed_shelf_after){{  $proposed_shelf_after->proposed_shelf_life_after_reconstitution_amount    }}<br> @endforeach </td></tr>
          



            <tr><td {{ $i=1 }}>Visual Description </td> <td>@foreach ($product_details as $visual_description){{  $visual_description->visual_description   }}<br>@endforeach</td></tr>
            <tr><td {{ $i=1 }}>Commercial Presentation   </td> <td> @foreach ($product_details as $commercial){{  $commercial->commercial_presentation  }}<br> @endforeach </td></tr>
          

            <tr><td {{ $i=1 }}>Container, closure and administration devices: </td> <td>@foreach ($product_details as $container){{  $container->container   }}<br>@endforeach</td></tr>
            <tr><td {{ $i=1 }}>Packaging and pack size   </td> <td> @foreach ( $product_details as $packaging_pack_size ){{ $packaging_pack_size->packaging   }}<br> @endforeach </td></tr>
          

            <tr><td {{ $i=1 }}>Category of Use </td> <td>@foreach ($product_details as $category_of_use){{ $category_of_use->category_use   }}<br>@endforeach</td></tr>
